...
const TreeTiTleRender = ({nodeData, showIcons: icons, titleOperate}: TreeTiTleRenderProps) => {
    ...
    return <div className="jrc-hover jrc-flex jrc-justify-between" onClick={(e: any) => titleClick(e, 'titleClick')}>{title}
                <div className="jrc-invisible jrc-hover-visible">
                    {showIcons?.includes('add') ? <Icon type="rc-jiahao" style={{ color: '#0479FE', marginLeft: 6}} onClick={(e: any) => titleClick(e, 'add')} /> : null}
                    {showIcons?.includes('update') ? <Icon type="rc-web-icon-" style={{ color: '#0479FE', marginLeft: 6}} onClick={(e: any) => titleClick(e, 'update')} /> : null}
                    {showIcons?.includes('del') ? <Icon type="rc-shanchu" style={{ color: '#0479FE', marginLeft: 6}} onClick={(e: any) => titleClick(e, 'del')} /> : null}
                </div>
            </div>
}
...
const WithSearchTreeWarp = ({title, onOperate, Icon: MIcon, children}: WithSearchTreeWarpProps) => {
    return <div>
       {(typeof title === 'boolean' && !title) ? null : <div className='jrc-flex jrc-justify-between jrc-align-middle' style={{fontSize: '14px', fontWeight: 500, background: '#F5F5F5', boxSizing: 'border-box', padding: '12px'}}>
            {title}{MIcon ? MIcon : <Icon type="rc-jiahao" style={{cursor: 'pointer'}} onClick={onOperate} />}
        </div>}
        <div style={{boxSizing: 'border-box', padding: '12px', background: '#fff',}}>{children}</div>
    </div>
}

function WithSearchTreeHook({treeData: pTreeData}: WithSearchTreeProps) {
    ...
    useEffect(() => {
        ...
        const { newTreeData } = handleTreeData(copyTreeData, ({title, ...rest}) => ({_title: title, title, ...rest}))
...
function WithSearchTree ({warpStyle, placeholder, onSearch, titleOperate, showIcons, treeData: pTreeData, noChildren, ...rest}: WithSearchTreeProps) {
    ...
    return (<div style={warpStyle}>
                ...
                />
                <Tree 
                    className={noChildren ? 'jrc-ant-tree-no-children' : ''} 
...
WithSearchTree.Warp = WithSearchTreeWarp
